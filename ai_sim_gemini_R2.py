...
def simulation(content: str) -> str:
    ...
    if "Error" in process_info or not process_info:
        return process_info  # Return the error message

    # 2. Use LLM to structure the process and identify steps and stream names
    print("Step 2: Structuring process steps and predicting stream names.")

    # Use structured output for reliable step parsing
    structured_llm = llm.with_structured_output(List[ProcessStep])
    ...
    try:
        # The response content will be a JSON string that conforms to the schema
        structured_steps: List[ProcessStep] = structured_llm.invoke([HumanMessage(content=structuring_prompt)])
    except Exception as e:
