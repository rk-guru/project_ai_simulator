...
def get_inlet_stream_tool(prompt: str) -> dict:
    """
    A tool to get the initial stream data from a user's prompt using an LLM.
    Args:
    ...
    # Define a prompt and JSON schema to extract parameters from the user's input
    system_prompt = f"""
You are an expert at extracting chemical stream parameters from natural language.
Your task is to parse the user's prompt and extract the temperature, pressure, flowrate, and chemical composition.
...
{{
    "type": "object",
    "properties": {{
        "temperature_C": {{"type": "number", "description": "The temperature in degrees Celsius."}},
        "pressure_bar": {{"type": "number", "description": "The pressure in bar."}},
        "flowrate_mol_s": {{"type": "number", "description": "The molar flowrate in mol/s."}},
        "chemicals": {{
            "type": "object",
            "description": "The chemical components and their mole fractions.",
            "additionalProperties": {{"type": "number"}}
        }}
    }},
    "required": ["temperature_C", "pressure_bar", "flowrate_mol_s", "chemicals"]
}}
"""
    response = llm.invoke([
    ...
    try:
        ...
        pressure_bar = data.get("pressure_bar", 1.0)
        flowrate_mol_s = data.get("flowrate_mol_s", 100.0)
        chemicals = data.get("chemicals", {"benzene": 0.5, "ethylene": 0.5, "ethyl benzene": 0})

        # Calculate moles from mole fraction and flowrate
        ...
        return stream(
            ...
            chemical_fraction={'benzene': 50, 'ethylene': 50, 'ethyl benzene': 0},
            flow=100.0
        )
...
def simulation(content: str) -> str:
    ...
    try:
        ...
        process_info = """
                 As a chemical engineer planning to simulate the styrene process, I have collected the following information about the process:

        **Process Overview:**

        * The styrene production process involves the reaction of benzene (C6H6) with ethylene (C2H4) in an equimolar feed.
        * The goal is to produce ethylbenzene (C6H5C2H5), a key intermediate in the production of styrene.

        **Process Conditions:**

        * Inlet stream:
        	+ Molar composition: 0.5 Benzene, 0.5 Ethylene
        	+ Initial temperature: 95°C
        	+ Pressure: 1.01325 bar
        	+ Molar flow rate: 9.419034 kmol/h
        	+ Mass flow rate: 500 kg/h
        * Pump:
        	+ Increases pressure from 1.01325 bar to 13 bar
        * Conversion reactor:
        	+ Ethyl benzene is produced with a 70% conversion rate based on benzene
        * Cooler:
        	+ Reduces temperature of product stream to 150°C
        * First Distillation Column:
        	+ Separates purified ethylbenzene as the bottom product from distillate containing unreacted ethylene and benzene
        ...
        * First Distillation Column"""
        ...
        print('=='*100)
        # print('process info =',process_info)
#         updated_response = RAG_CHAIN.invoke([
...
#         ])
        print('---'*100)
        updated_response ="""Based on the provided context, I'll modify the process steps according to the conditions mentioned in the system message. Here's the revised process:

**Step 1: Heater**
- Inlet stream composition: equi-molar feed of benzene (0.5) and ethylene (0.5)
...
**Step 2: Pump**
- Inlet stream composition: equi-molar feed of benzene (0.5) and ethylene (0.5)
- Temperature: Same as Step 1 (95°C)
- Pressure increase from 1.01325 bar to 13 bar
- Molar flow rate remains the same

**Step 3: Conversion Reactor**
- Inlet stream composition: equi-molar feed of benzene (0.5) and ethylene (0.5)
- Temperature: Same as Step 2
- Pressure: 13 bar
- 70% conversion rate based on benzene, producing ethyl benzene

**Step 4: Cooler**
- Inlet stream composition: ethyl benzene, unreacted ethylene, and benzene
- Initial temperature: 150°C (from reactor)
- Final temperature: 150°C (cooler outlet)

**Step 5: Pump**
- Pressure increase from cooler outlet pressure to distillation column pressure (not specified)

**Step 6: Distillation Column**
- Separates purified ethyl benzene as the bottom product
- Distillate contains unreacted ethylene and benzene


Note that I've assumed the second pump is used after the cooler, but since it's not explicitly mentioned where the increased pressure from this pump is needed, its exact location could be different based on actual process requirements.
"""
        print('updated_response =', updated_response)
        # number_of_stages = RAG_CHAIN.invoke([
        #     SystemMessage(content=f'Get the number of steps in the process . return only the number '),
        #     HumanMessage(content=updated_response)
        # ])
        number_of_stages ='The number of steps in the process is 6.'
        print('///' * 100)
        print('number_of_stages',number_of_stages)
        # stream_names = RAG_CHAIN.invoke([
        #     SystemMessage(
        #         content='''you are a chemical simulation expert ,you need to create outstream names based on the following conditions  the names should be of format Equipment followed by number like Heater_1_outlet , pump_1_outlet, and so on
        #          for flash and distillation column the outlet have 2 stream so Flash_1_liquid_outlet , Flash_1_vapor_outlet and same for distillation .
        #          if the same equipment type is availabe more than one like 2 heater the first heater will be named as Heater_1_outlet and second named as Heater_2_outlet,give the response only as a list of outet streams '''),
        #     HumanMessage(content=updated_response)
        # ])
        stream_names ="""
        Based on the provided context and conditions, here is the list of outlet streams:

1. Heater_1_outlet
2. Pump_1_outlet
3. Conversion_Reactor_1_outlet (containing ethyl benzene, unreacted ethylene, and benzene)
4. Cooler_1_outlet (temperature: 150°C)
5. Pump_2_outlet (increased pressure to distillation column pressure)
6. Distillation_Column_1_liquid_outlet (purified ethyl benzene)
7. Distillation_Column_1_vapor_outlet (distillate containing unreacted ethylene and benzene)"""
        print('+++' * 100)
        print('stream_names',stream_names)
        # feed_stream=RAG_CHAIN.invoke([
        #     SystemMessage(
        ...
        #     HumanMessage(content=updated_response)
        # ])
        feed_stream="""You are interested in knowing more about the feed information for the styrene production process. The inlet stream composition is an equi-molar feed of Benzene (0.5) and Ethylene (0.5). Here's a summary of the relevant feed information:

* Chemical Composition: Benzene (0.5) and Ethylene (0.5)
* Temperature: Initially 95°C
* Pressure: Constantly maintained at 1.01325 bar until increased to 13 bar after passing through a pump.
* Flow Rates:
	+ Molar flow rate: 9.419034 kmol/h
	+ Mass flow rate: 500 kg/h"""
        print('---' * 100)
        print('feed stream,',feed_stream)
        current_stream = get_inlet_stream_tool.func(prompt=feed_stream)
        print('*+*' * 100)
